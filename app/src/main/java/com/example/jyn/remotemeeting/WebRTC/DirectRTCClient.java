 ...
 * 이렇게하면 외부 서버가 필요하지 않습니다. 이 클래스는 루프백 연결을 지원하지 않습니다.
 */
public class DirectRTCClient implements AppRTCClient, TCPChannelClient.TCPChannelEvents {
    ...
    // 방 ID가 IP처럼 보이는 지 확인하는 데 사용되는 정규식 패턴입니다.
    ...
    // room 상태의 모든 변경은 looper thread 내부에서 이루어져야합니다.
    ...
    /**
     * Connects to the room, roomId in connectionsParameters is required.
     * roomId must be a valid IP address matching IP_PATTERN.
     * <p>
     * room에 연결합니다. 연결에 room이 있어야합니다. 매개 변수가 필요합니다.
     * roomId는 IP_PATTERN과 일치하는 유효한 IP 주소 여야합니다.
     */
    @Override
